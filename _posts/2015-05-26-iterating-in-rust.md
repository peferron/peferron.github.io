...
It would work quite well in our example because there are only 32 such values. But we can do better: not allocate a `Vec` at all, and lazily generate only as many numbers as we need.
